...
import {IBookCart, ICartSliceInitialState} from "./types/cartBookType";
...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    removeCart: (state: ICartSliceInitialState, action: PayloadAction<string>) => {
      state.cart = state.cart
        .filter((book) => book.isbn13 !== action.payload)
    }
  }
})
